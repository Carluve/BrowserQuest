/* Ported from server/js/worldserver.js (uses global `log`).
 *
 * Changes vs. original:
 *  - `run()` takes the already-parsed map JSON (no file path) and no longer
 *    starts a setInterval loop.
 *  - The per-tick work is exposed as `tick()`, driven by the Durable Object's
 *    Alarms API (see src/world-do.ts).
 */
...
var World = cls.Class.extend({
    ...
    run: function (mapJSON) {
        var self = this;

        this.map = new Map(mapJSON);
    ...
    // One iteration of the game loop, driven by the Durable Object alarm.
    tick: function () {
        this.processGroups();
        this.processQueues();
